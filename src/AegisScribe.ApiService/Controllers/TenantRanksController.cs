...
// A community's own rank ladder — "Raider", "Trial", "Social". Not the in-game ranks Blizzard reports;
// those are GuildMember.BlizzardRank, global and read-only (tenancy.md).
//
...
// resolution middleware, never a 403, because a 403 would confirm the community exists.
[ApiController]
[ApiVersion("1.0")]
...
public class TenantRanksController(ITenantRankFacade rankFacade) : ControllerBase
{
    ...
    // No Location header, because there is no GET /{rankId} route to point one at — no screen fetches a
    // single rank, and every route is a promise for the life of v1 (api-contract.md).
    [HttpPost]
    [Idempotent]
    ...
    // 204 whether or not the rank was there: DELETE is idempotent (api-contract.md), and the query
    // filter makes a delete of another community's rank id match zero rows rather than theirs.
    //
    // The one case that is NOT 204 is a rank roster entries still hold — a 409, because the alternative
    // is silently un-ranking everyone who held it.
