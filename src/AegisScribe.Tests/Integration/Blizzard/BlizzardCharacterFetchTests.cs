...
// BlizzardRateLimitHandler are wired exactly as BlizzardServiceCollectionExtensions wires them, because
// "every gateway method sends a bearer header and takes a lease" is a claim about the composition.
//
// Response bodies come from Integration/Blizzard/Fixtures — see the README there.
public class BlizzardCharacterFetchTests
{
    ...
    {
        // The namespace is the assertion that matters most in this file: static- would return a 404 that
        // this gateway reports as "no such character". Locale is a query parameter too — Blizzard
    ...
    {
        // dynamic-, not static-: realms move between connected-realm groups. A static- namespace 404s,
        // which reads as "no such realm" — and a character on a realm we cannot resolve disappears too.
    ...
    {
        // The connected-realm id exists only as the last path segment of connected_realm.href. Reading it
        // is not the same as following it: no second request is made.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("realm.json"));
        ...
        // The caller's short region code, not Blizzard's prose "North America".
    ...
    {
        // A source id of 0 shared by every realm we failed to parse is the kind of corruption only
        // noticed once it is in every row, so this fails loudly at the boundary instead.
    ...
    {
        // Why the catalogue goes through connected realms rather than /data/wow/realm/index: one document
        // yields several realms AND the connected-realm id, as a field rather than a link.
        ...
        // Its own id, distinct from the group id above — what lets a renamed realm update in place.
    ...
    {
        // A group can be dissolved between the index call and this one — a normal answer for a pass that
        // walks ~100 of them, not a failure of the pass.
        var harness = Harness.Returning(
            HttpStatusCode.NotFound,
            """{"code":404,"type":"BLZWEBAPI00000404","detail":"Not Found"}""");
    ...
    {
        // An empty list here would look like "this region has no realms", and the pass would write
    ...
    {
        // The trap this integration keeps stepping on: guild endpoints sit under /data/wow/ like Game
        // Data but take profile-, and the wrong namespace 404s in a way that looks like a typo in the
        // guild name.
    ...
    {
        // The roster response carries the guild object alongside the members, so linking a guild never
        // needs a separate guild-summary request.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("guild-roster.json"));
    ...
    public async Task FetchGuildRoster_CarriesNoItemLevelOrGear()
    {
        ...
        // call, gear is two per member. A future reader tempted to "just fill in the item levels here"
        // should meet this test first.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("guild-roster.json"));
    ...
    {
        // The fixture contains a class id 99 that does not exist. For a single fetch that is a hard
        // failure; for a batch it must cost one member, or one unrepresentable member makes a 400-person
        // guild permanently unsyncable.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("guild-roster.json"));
    ...
    {
        // Plenty of EU characters have an accent in their name. An unencoded one is a malformed URL, not
        // a 404, so it fails in a way that looks nothing like the cause.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("character-summary.json"));
    ...
    {
        // Blizzard disallowed the query-parameter form in 2024 (external.md). The header comes from
        // BlizzardAuthHandler, which is why that handler is in this pipeline rather than stubbed out.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("character-summary.json"));
    ...
    {
        // average_item_level counts what is sitting in the bags; equipped_item_level is the number an
        // armory shows. The fixture has 623 and 620 precisely so a mix-up cannot pass.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("character-summary.json"));
    ...
    public async Task FetchCharacter_LeavesTheRealmAndEquipmentForTheCallerToResolve()
    {
        ...
        // fabricate Realm.BlizzardConnectedRealmId, which the erasure routine keys on.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("character-summary.json"));
    ...
    {
        // A character below the specialization level has no active_spec key at all, and an unguilded one
        // has no guild key. Neither is null; both are absent.
    ...
    public async Task FetchEquipment_MapsSlotsAndQualitiesFromTheInvariantTypeTokens()
    {
        ...
        // Why ItemQuality has a Heirloom member: Blizzard sends HEIRLOOM on any levelling alt.
    ...
    {
        // SHIRT and TABARD are in the captured body and have no EquipmentSlot member. Dropping them is
        // the decision, so it is asserted rather than left to be rediscovered.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("character-equipment.json"));
    ...
    {
        // The response carries a media href, not a filename. Resolving it is one call per item, which
        // would turn one character fetch into seventeen.
        var harness = Harness.Returning(HttpStatusCode.OK, Fixture("character-equipment.json"));
    ...
    {
        // An empty list is a real answer about a real character, and not the same as a 404.
    ...
    {
        // A character that does not exist is a normal answer, not an exception. Blizzard sends a JSON
        // error body with it, which this path must not try to map.
        var harness = Harness.Returning(
            HttpStatusCode.NotFound,
            """{"code":404,"type":"BLZWEBAPI00000404","detail":"Not Found"}""");
    ...
    {
        // The distinction BlizzardUnavailableException exists for. Returning null here would cache an
    ...
    {
        // A proxy in front of Blizzard serving an HTML error page with a 200. It must not surface as a
        // raw JsonException from somewhere deep in the DataLayer.
    ...
    {
        // Missing credentials degrade rather than crash (external.md), and the caller falls back to
        // stored data. What it must not do is spend a request discovering there is no token.
    ...
    {
        // The 429 path end to end through the real limiter: a throttled response is never mistaken for a
        // missing character, and the retry hint actually stops the next call from leaving. Retry-After is
        // 30 seconds against a MaxThrottleWait of 5, so the second caller is refused a lease rather than
        // made to wait it out.
    ...
    public async Task FetchCharacter_WithAFactionThisApplicationCannotRepresent_ThrowsRatherThanGuessing()
    {
        ...
        // it. Mutated from the captured body, so only the field under test differs.
    ...
    {
        // Hand-written on purpose: this is a future expansion adding a class id we have no member for,
        // which no capture taken today can contain. Failing means the caller falls back to the stored
        // row; guessing would persist a character as the wrong class, and ClassColorHex would then throw
        // somewhere the cause is invisible.
        const string body = """
            {
              "id": 176946929,
              "name": "Aldric",
