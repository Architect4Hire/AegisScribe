...
//
// The distinction this type exists to preserve: a 404 returns null, because a character that does not
// exist is a normal answer, and everything else throws. Collapsing the two would let the cache-first
// read (6.4) treat an outage as "no such character", cache that, and hide a live character behind an
// empty page until someone noticed. Callers catch this and fall back to the stored row.
